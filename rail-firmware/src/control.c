...
static void profile_step(float target, float *velocity, float *acceleration)
{
...
static float stopping_distance(float velocity, float acceleration)
{
    float position = 0.0f;
    if (velocity <= 0.0f)
    {
...
static float position_safe_target(float target)
{
    ...
    profile_step(target, &candidate_velocity, &candidate_acceleration);
    ...
    if (candidate_velocity > 0.0f)
    {
        ...
        if (next_motion + stopping_distance(candidate_velocity,
...
static void run_control_cycle(void)
{
        ...
        state == STATE_STOPPING)
    {
        ...
        profile_step(target, &command_velocity_mm_s,
